...
Rostam Media is a way of downloading content like files and videos from satelite TVs. It provides a one-way Internet by integrating data inside ts packets. For extracring the data, you need to record the TV channel in *.ts format and extract from the ts file using the extractor software on your computer. Rostam Media's client (EQSat GUI) is only available on Windows (Not available for Windows 7). So I made this client for other platforms in C++.
